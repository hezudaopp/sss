 ...
 * @package Core
 * @version $Id: FileUploader.php 947 2007-09-29 16:02:15Z dualface $
 */
                ...
                if (is_array($struct['error'])) {
                    ...
                    for ($i = 0; $i < count($struct['error']); $i++) {
                        ...
                        if ($struct['error'][$i] != UPLOAD_ERR_NO_FILE) {
                            $arr[] =& new FLEA_Helper_UploadFile($struct, $field, $i);
                            if (!$cascade) {
...
class FLEA_Helper_UploadFile
{
     ...
     * @return FLEA_Helper_UploadFile
     */
    function FLEA_Helper_UploadFile($struct, $name, $ix = false)
    {
        if ($ix !== false) {
            $s = array(
                'name' => $struct['name'][$ix],
