...
class SSS_MonitorExcelReader extends SSS_ExcelReader {
	
	private $fileId = null;
	
	public function __construct(){
		$this->tableName = 'sss_monitor';
	...
	function saveFileInfo(&$record){
		global $file;
		$query = "insert into sss_monitor_file set filename = '{$file->getFilename()}', record = '$record', uploadTime = now()";
		DB::query($query);
	...
	public function myHaveUploaded(){
		$query = "select filename, uploadTime from sss_monitor_file where filename = '$this->filename'";
...
/*		if(!empty($lackedColumns)){
	...
	public function myReadIntoDB(){
		...
		try{
			...
			throw $e;
			/*$query = 'delete from sss_monitor_file where id = '. $this->fileId;
			DB::query($query);
			throw $e;*/
		}
	}
