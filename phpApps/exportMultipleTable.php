...
/*		
	...
	if(isset($_GET['orderNumber'])){
		...
		$query = "select id, facheDate, facheNumber, materialCode, shipsClassification, material, 
			...
			uploadTime, filename, phase, orderNumber, orderSubitemNumber, unitPrice, batchNumber, purchaseNumber, destination, storePlace,
			certificateNumber, checkoutBatch , materialNumber, consignmentBatch
			from sss_fache where materialCode = '{$keyname}' and phase ='入库' order by thickness, width, length";
	}	
	DB::query($query);
...
/*		$inSumCount = 0;
	...
	$exp->setRukuSumCount($inSumCount);
	$exp->setRukuSumWeight($inSumWeight);
	...
	if(isset($_GET['orderNumber'])){
		...
		uploadTime, filename, phase, orderNumber, orderSubitemNumber, unitPrice, batchNumber, purchaseNumber, destination, storePlace,
		certificateNumber, checkoutBatch, materialNumber, consignmentBatch
		from sss_fache where materialCode = '{$keyname}' and phase ='出库' order by thickness, width, length";
	}
	DB::query($query);
	...
	if(DB::num_rows() > 0){
		...
		while($row = $result->fetch_assoc()){
			...
			array_push($checkoutBatchs,$row['checkoutBatch']);
			array_push($materialNumbers,$row['materialNumber']);
			array_push($consignmentBatchs,$row['consignmnetBatch']);
		}
		/*foreach($counts as $count){
			$outSumCount += $count;
		}
...
/*		$smarty->assign(array('fachuan_fachuanDate' => $fachuanDates,
	...
	*/	
		...
		$data = array($fachuanDates, $materialCodes, $fachuanNumbers, $shipsClassifications,
					...
					$weights, $remarks, $uploadTimes, $filenames, $orderNumbers, $orderSubitemNumbers,
					$unitPrices, $batchNumbers, $materialNumbers, $purchaseNumbers, $destinations, $storePlaces,$certificateNumbers,$checkoutBatchs,$consignmentBatchs,$phases);
		$exp->setChukuSumCount($outSumCount);
		$exp->setChukuSumWeight($outSumWeight);
		$exp->insertChukuFachuanData($data);
	}
	...
	//最后是直接销售的，跟入库的差不多
		...
		uploadTime, filename, phase, orderNumber, orderSubitemNumber, unitPrice, batchNumber, purchaseNumber, destination, storePlace,
		certificateNumber, checkoutBatch, materialNumber, consignmentBatch
		from sss_fache where materialCode = '{$keyname}' and phase ='销售' order by thickness, width, length";
	}
	DB::query($query);
...
/*		$smarty->assign(array('directFache_facheDate' => $facheDates,
	...
	$data = array($facheDates, $materialCodes, $facheNumbers, $shipsClassifications,
				...
				$weights, $remarks, $uploadTimes, $filenames, $orderNumbers, $orderSubitemNumbers,
				$unitPrices, $batchNumbers, $materialNumbers, $purchaseNumbers, $destinations, $storePlaces, $certificateNumbers, $checkoutBatchs, $consignmentBatchs, $phases);
	$exp->setSellSumCount($directSumCount);
	$exp->setSellSumWeight($directSumWeight);
	$exp->insertSellData($data);
	}
