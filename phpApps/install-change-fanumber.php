...
if (! isset ( $_POST ['password'] )) {
	?>
			...
			<form method="POST" action="install-change-fanumber.php">
<table>
	<tr>
...
$query = "alter table sss_fache change facheNumber facheNumber varchar(8);";
@$db->query ( $query );
if (mysqli_errno ( $db )) {
	die ( '查询语句错误：' . $query . ',错误原因：' . mysqli_error ( $db ) . '<br />' );
}

$query = "alter table sss_fachuan change fachuanNumber fachuanNumber varchar(8);";
@$db->query ( $query );
if (mysqli_errno ( $db )) {
	die ( '查询语句错误：' . $query . ',错误原因：' . mysqli_error ( $db ) . '<br />' );
}
