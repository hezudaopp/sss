...
if (! isset ( $_POST ['password'] )) {
	?>
			...
			<form method="POST" action="install-optimize-kuzhong.php">
<table>
	<tr>
...
try {
	...
	$query = 'alter table sss_main add index main_mc_index(materialCode);';
	DB::query ( $query );
	echo "<span style='color:green'>为 sss_main添加mc索引成功</span><br />";
	$query = 'alter table sss_fache add index fache_mc_index(materialCode)';
	DB::query ( $query );
	echo "<span style='color:green'>为sss_fache添加mc索引成功</span><br />";
	$query = 'alter table sss_fachuan add index fachuan_mc_index(materialCode)';
	DB::query ( $query );
	echo "<span style='color:green'>为sss_fachuan添加mc索引成功</span><br /><br />";
	
	echo "<span style='color:green'>记录表安装成功，您可以正常使用监控程序了</span><br />";
