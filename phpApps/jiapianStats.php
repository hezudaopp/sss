...
$smarty->assign('jiapian', true);
try{
if(!isset($_GET['orderNumber'])){
	$account = new ChineseCharsAccountant(false, 'jiapian');
	$account->execute();
	$smarty->assign( array(
	...
	$query = "select id, facheDate, facheNumber, materialCode, shipsClassification, material,
		...
		unitPrice, batchNumber, purchaseNumber, destination, storePlace,
		certificateNumber, checkoutBatch, maaterialNumber, consignmentBatch
		from sss_fache
		where materialCode = '加片'
			and orderNumber = '$orderNumber'
			and orderSubitemNumber = '$orderSubitemNumber'
