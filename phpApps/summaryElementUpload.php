...
require('../includes/SSS_SummaryElementExcelReader.class.php');
 ...
 ************************************************************/
$formFilename = 'summaryElementFile';
...
try{
	...
	$sssReader = new SSS_SummaryElementExcelReader();
	...
	LogInserter::logForInsert("导入汇总因素文件：{$file->getFilename()}");
}catch(DBQueryException $e){
	$smarty->assign('errMsg', $e);
	$smarty->assign('errorType', 'summaryElementUpload');
	$smarty->assign("errTitle", '上传汇总因素文件失败1');
	$smarty->display('error.html');
	...
	$smarty->assign('errMsg', $e);
	$smarty->assign('errorType', 'summaryElementUpload');
	$smarty->assign("errTitle", '上传汇总因素文件失败2');
	$smarty->display('error.html');
	rollback();
	exit();
}
...
$smarty->assign("successTitle", '上传汇总因素文件成功');
$smarty->assign('successType', 'summaryElementUpload');
if(isset($warnMsg)){
